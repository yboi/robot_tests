...
Створити тендер
  ...
  Sleep  20
  Wait Until Page Contains Element    xpath=//*[text()="Оголосити"]   10
  Click Element                       xpath=//*[text()="Оголосити"]
  Click Element                       xpath=(//*[text()="Оголосити"])[3]
  Sleep  30
  Reload page   #тому що сторінка постійно грузиться
  Wait Until Page Contains            Картка закупівлі № UA   30
...
Пошук тендера по ідентифікатору
  ...
  Sleep  5
  Click Element                      xpath=(//div[@class='ui-row-toggler ui-icon ui-icon-circle-triangle-e'])[1]
  sleep  10
...
Завантажити документ
  ...
  Click Element                      id=mForm:bSave
  Reload Page   #це необхідно так як нам відображає вікно про помилку, хоча все документ доданий
  Capture Page Screenshot
...
отримати інформацію про items[0].deliveryLocation.latitude
  ...
  ${return_value}=   Get Substring  ${return_value}  0   10  #поки використовую хард код, так як постійно одні і тіж дані з градусами
  [return]  ${return_value}

отримати інформацію про items[0].deliveryLocation.longitude
  ${return_value}=   Отримати значення із поля і показати на сторінці  items[0].deliveryLocation.longitude
  ${return_value}=   Get Substring  ${return_value}   11   #поки використовую хард код, так як постійно одні і тіж дані з градусами
  [return]  ${return_value}
...
отримати інформацію про items[0].unit.name
  ...
  ${return_value}=   Remove String   ${return_value.split(' ')[1]}  и
  [return]  ${return_value}
