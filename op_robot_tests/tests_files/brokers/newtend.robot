...
отримати інформацію про items[0].deliveryAddress.streetAddress
  ${Delivery_Address}=   отримати текст із поля і показати на сторінці   items[0].deliveryAddress
  ${Delivery_Address}=    Remove String   ${Delivery_Address.split(' ',6)[6]}    ,
...
отримати інформацію про items[0].unit.name
  ...
  ${unit_name}=   Run keyword if    '${unit_name}' == 'кілограми'   Convert To String  кілограм
  [return]  ${unit_name}

отримати інформацію про items[0].unit.code
  ${unit_code}=   отримати текст із поля і показати на сторінці   items[0].unit.name
  ${unit_code}=   Run keyword if    '${unit_code}' == 'кілограми'   Convert To String  KGM
  [return]  ${unit_code}
